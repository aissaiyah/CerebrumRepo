...
public class TestTubeFiller : ObjectMessageHandlerBase
{
    ...
    public float fillAmount = 0.5f;//value 0-1
    public GameObject LiquidHolder;
    public Material mat;

    public override bool HandleMessage(string msg, string param, out string retString)
    {
        DebugConsole.print("TestTubeFiller:HandleMessage: "+ msg + " " + param);
        retString = null;
        bool retv = base.HandleMessage(msg,param,out retString);
        if (commandFound)
            return retv;
        DebugConsole.print("TestTubeFiller:HandleMessage:2: "+ msg + " " + param);


        /*  COMMANDS */

        if (msg == "fillamount"){
            DebugConsole.print("fillamount "+ param);
